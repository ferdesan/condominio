...
function chooseArea(name: string): void {
  selectOption(dialog().getByLabelText('Área comum'), name);
}
...
function fillRange(start: string, end: string): void {
  const view = dialog();
  fireChange(view.getByLabelText('Início'), start);
  fireChange(view.getByLabelText('Término'), end);
}
...
describe('Formulário de reserva', () => {
  it('IT-138: escolhe a área, ve as regras, envia um período válido e a reserva aparece nas duas visões', async () => {
    await openValidForm();

    // As restricoes da area aparecem antes do envio.
    const rules = within(dialog().getByRole('region', { name: 'Regras da área' }));
    expect(rules.getByText('Funcionamento das 08:00 as 22:00.')).toBeInTheDocument();
    expect(rules.getByText('Duração mínima de 2h.')).toBeInTheDocument();
    expect(rules.getByText('Duração máxima de 6h.')).toBeInTheDocument();
    expect(rules.getByText('Antecedência máxima de 60 dias.')).toBeInTheDocument();
  ...
  it('IT-139: término igual ao início e recusado no campo do término, sem requisição', async () => {
    await openValidForm();
    ...
    await waitFor(() =>
      expect(dialog().getByText('O término deve ser posterior ao início.')).toBeInTheDocument(),
    );
  ...
  it('IT-140: um início no passado e recusado sem requisição', async () => {
    await openValidForm();
    ...
    await waitFor(() =>
      expect(
        dialog().getByText('Não e possível reservar uma data no passado.'),
      ).toBeInTheDocument(),
    );
    expect(postCalls()).toHaveLength(0);
  });

  it('IT-141: um início alem do limite de antecedência e recusado informando o limite', async () => {
    await openValidForm();
    ...
    await waitFor(() =>
      expect(
        dialog().getByText('Reservas podem ser feitas com no máximo 60 dias de antecedência.'),
      ).toBeInTheDocument(),
    );
    expect(postCalls()).toHaveLength(0);
  });

  it('IT-142: durações abaixo do mínimo e acima do máximo sao recusadas, cada uma com seu limite', async () => {
    await openValidForm();

    fillRange(ahead(7, 18), ahead(7, 19));
    submit();
    await waitFor(() =>
      expect(dialog().getByText('A reserva mínima para esta área e de 2h.')).toBeInTheDocument(),
    );

    fillRange(ahead(7, 12), ahead(7, 19));
    submit();
    await waitFor(() =>
      expect(dialog().getByText('A reserva máxima para esta área e de 6h.')).toBeInTheDocument(),
    );

    expect(postCalls()).toHaveLength(0);
  });

  it('IT-143: um dia da semana que a área não permite e recusado', async () => {
    renderWithProviders(<ReservationsPage />);
    await screen.findByText('Carlos Pereira');
    await openForm();
    chooseArea('Quadra');
    ...
    await waitFor(() =>
      expect(
        dialog().getByText('A área comum não esta disponível neste dia da semana.'),
      ).toBeInTheDocument(),
    );
    expect(postCalls()).toHaveLength(0);
  });

  it('IT-144: um período fora do horário de funcionamento e recusado informando a janela', async () => {
    await openValidForm();
  ...
  it('IT-145: um 409 de sobreposição aparece no formulário e preserva o que foi digitado', async () => {
    await openValidForm();

    const message = 'Já existe uma reserva para esta área neste horário.';
    ...
    expect(dialog().getByLabelText('Início')).toHaveValue(start);
  });

  it('IT-146: um 409 de intervalo mínimo aparece com a mensagem do servidor', async () => {
    await openValidForm();

    const message = 'Cada unidade pode reservar esta área a cada 30 dia(s).';
    mockPost.mockRejectedValueOnce(new ApiError(message, 409, 'BUSINESS_RULE'));
  ...
  it('IT-147: convidados acima da capacidade sao recusados', async () => {
    ...
    await waitFor(() =>
      expect(dialog().getByText('A área comporta no máximo 50 pessoas.')).toBeInTheDocument(),
    );
    expect(postCalls()).toHaveLength(0);
  });

  it('IT-148: uma área indisponível não e oferecida no seletor', async () => {
    serveApi({
      areas: [
        SALAO,
    ...
    const trigger = dialog().getByLabelText('Área comum');
    trigger.focus();
  ...
  it('IT-149: um 422 de campo aparece no campo e um 409 no formulário, nenhum em toast', async () => {
    await openValidForm();

    mockPost.mockRejectedValueOnce(
      new ApiError('Dados invalidos.', 422, 'VALIDATION', [
        { field: 'guestsCount', message: 'Número de convidados inválido.' },
      ]),
    );
    ...
    await waitFor(() =>
      expect(dialog().getByText('Número de convidados inválido.')).toBeInTheDocument(),
    );
    ...
    mockPost.mockRejectedValueOnce(new ApiError('Conflito de horário.', 409, 'CONFLICT'));
    submit();

    await waitFor(() =>
      expect(dialog().getByRole('alert')).toHaveTextContent('Conflito de horário.'),
    );
    expect(mockToastError).not.toHaveBeenCalled();
  });

  it('IT-150: dois envios para o mesmo horário produzem um sucesso e um conflito', async () => {
    await openValidForm();
  ...
  it('IT-151: dois cliques seguidos no envio produzem um único POST', async () => {
    await openValidForm();
  ...
  it('IT-152: um condomínio sem áreas comuns explica que uma área e necessária antes', async () => {
    ...
    expect(
      dialog().getByText(/ainda não tem áreas comuns disponíveis para reserva/i),
    ).toBeInTheDocument();
